	...
	uniform float3 translate, uniform float3 scale)
{
	...
	oTexcoord2 = float4(oTexcoord1.xyz * scale * mSunRadius + SunDirection * translate, 1); // ponytail: moon at light source
	...
	uniform sampler source) : COLOR
{
	...
	diffuse *= saturate(dot(normalize(normal), -SunDirection) + 0.1) * 0.4;	
